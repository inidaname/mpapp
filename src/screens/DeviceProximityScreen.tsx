...
const DeviceProximityScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // Clean up broadcasting if user leaves screen
  useEffect(() => {
    return () => {
      stopBroadcasting();
    };
...
useEffect(() => {
    ...
    const subscription = bleEventEmitter.addListener("onPaymentResponse", (event) => {
      ...
      if (event.message === "ACCEPTED") {
        ...
        // Update UI or Local Database here
      }
    });
  ...
  const startBroadcasting = async (amountValue: string) => {
    ...
    BLEAdvertiser.startBroadcast(username, Number.parseFloat(amount), SERVICE_UUID, WRITE_CHAR_UUID_STR);
    console.log("Broadcasting", "Device is now visible to others nearby.");

      setIsBroadcasting(true);
      console.log(`Broadcasting payment: ${amountValue}`);
    } catch (error) {
  ...
  return (
    <View className="justify-between items-center flex-1 bg-white">
      ...
      <View className="w-full px-6 mt-8 items-center justify-center">
        {isBroadcasting
          ? (
            // --- BROADCASTING STATE UI ---
            <View className="items-center justify-center bg-blue-50 p-6 rounded-full w-64 h-64 border-4 border-blue-100">
              <ActivityIndicator size="large" color="#3B82F6" />
              <AppText className="text-xl mt-4 font-bold text-blue-600">
                Broadcasting...
              </AppText>
              <AppText className="text-lg font-medium text-gray-600 mt-2">
                ${amount}
              </AppText>
              <AppText className="text-xs text-center text-gray-400 mt-2">
                Hold near the receiver device
              </AppText>
            </View>
          )
          : (
            ...
            <>
              ...
              <AppText className="text-md font-thin text-center mt-2 px-4">
                Enable your phone's Bluetooth to connect with the recipient
                device.
              </AppText>
            </>
          )}
      ...
      <View className="px-4 w-full flex-row justify-between mb-10 gap-4 mt-10">
        {/* If broadcasting, show STOP button, else show standard buttons */}
        {isBroadcasting
          ? (
            <Pressable
              className="bg-red-500 flex-1 items-center justify-center px-6 py-3 rounded-lg"
          ...
          : (
            <>
              <Pressable
                className="bg-brand-700 flex-1 items-center justify-center px-6 py-3 rounded-lg"
              ...
              <Pressable
                ...
                style={{ borderWidth: 1 }}
                className="border-brand-700 border-1 px-6 py-3 flex-1 rounded-lg items-center justify-center"
              >
      ...
      >
        <View className="flex-1 bg-black/50 items-center justify-center p-6">
          <View className="bg-white p-6 rounded-xl w-full max-w-sm shadow-xl">
            <AppText className="text-lg font-medium mb-4">
              Enter Amount to Pay
            </AppText>
            ...
            <View className="flex-row justify-end space-x-4 gap-4">
              <Pressable
                onPress={closeModal}
                className="px-4 py-3 rounded-lg bg-gray-200 flex-1 items-center"
              >
              ...
              <Pressable
                ...
                className="px-4 py-3 rounded-lg bg-blue-500 flex-1 items-center"
              >
